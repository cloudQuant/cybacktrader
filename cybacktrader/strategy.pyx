...
from cybacktrader.utils.py3 import (filter, keys, integer_types, iteritems, itervalues,
                        map, MAXINT, string_types, with_metaclass)

import cybacktrader as bt
from cybacktrader.lineiterator import LineIterator, StrategyBase
from cybacktrader.lineroot import LineSingle
from cybacktrader.lineseries import LineSeriesStub
from cybacktrader.metabase import ItemCollection, findowner
from cybacktrader.trade import Trade
from cybacktrader.utils import OrderedDict, AutoOrderedDict, AutoDictList
...
class MetaStrategy(StrategyBase.__class__):
    ...
    def donew(cls, *args, **kwargs):
        ...
        # Check if cerebro is explicitly passed (for Cython compatibility)
        cerebro = kwargs.pop('_cerebro', None)
        if cerebro is None:
            # Fallback to findowner for backward compatibility
...
class Strategy(with_metaclass(MetaStrategy, StrategyBase)):
    ...
    def _addobserver(self, multi, obscls, *obsargs, **obskwargs):
        ...
        # Pass owner explicitly for Cython compatibility
        obskwargs['_owner'] = self
