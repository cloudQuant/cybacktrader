...
from cybacktrader.indicators.deviation import StdDev
...
class BollingerBandsPct(BollingerBands):
    ...
    def once(self, int start, int end):
        ...
        cdef double[:] top = self.lines.top.array
        cdef double[:] bot = self.lines.bot.array
        cdef double denom
        with nogil:
            for i in range(s, e):
                denom = top[i] - bot[i]
                dst[i] = (cur[i] - bot[i]) / denom if denom != 0.0 else 0.0