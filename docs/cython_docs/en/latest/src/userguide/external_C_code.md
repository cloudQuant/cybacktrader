...
Interfacing with External C Code — Cython 3.2.0b1 documentation
...
[here](/en/stable/src/userguide/external_C_code.html "/en/stable/src/userguide/external_C_code.html").

### Navigation

* [next](source_files_and_compilation.html "Source Files and Compilation")
* [previous](sharing_declarations.html "Sharing Declarations Between Cython Modules") |
...
# Interfacing with External C Code[¶](#interfacing-with-external-c-code "Link to this heading")
...
[embedding Python](https://web.archive.org/web/20120225082358/http://www.freenet.org.nz:80/python/embeddingpyrex/ "https://web.archive.org/web/20120225082358/http://www.freenet.org.nz:80/python/embeddingpyrex/") in another application as a scripting language. Just as a
...
## External declarations[¶](#external-declarations "Link to this heading")
...
### Referencing C header files[¶](#referencing-c-header-files "Link to this heading")
...
1. Leave out any platform-specific extensions to C declarations such as
   `__declspec()`.
2. If the header file declares a big struct and you only want to use a few
...
3. If the header file uses `typedef` names such as `word` to refer
   to platform-dependent flavours of numeric types, you will need a
   corresponding [`ctypedef`](language_basics.html#ctypedef "language_basics.html#ctypedef") statement, but you don’t need to match
   ...
   will work okay whatever the actual size of a `word` is (provided the header
   file defines it correctly). Conversion to and from Python types, if any, will also
   be used for this new type.
4. If the header file uses macros to define constants, translate them into a
   normal external variable declaration. You can also declare them as an
   [`enum`](language_basics.html#enum "language_basics.html#enum") if they contain normal int values. Note that
   Cython considers [`enum`](language_basics.html#enum "language_basics.html#enum") to be equivalent to int, so do
   not do this for non-int values.
5. If the header file defines a function using a macro, declare it as though
...
### Implementing functions in C[¶](#implementing-functions-in-c "Link to this heading")
...
### Styles of struct, union and enum declaration[¶](#styles-of-struct-union-and-enum-declaration "Link to this heading")
...
the declaration with [`ctypedef`](language_basics.html#ctypedef "language_basics.html#ctypedef"), as illustrated below.
...
cdef struct Foo:
  ...
```  Cython will refer to the type as `struct Foo` in
the generated C code.  ```
typedef struct {
  ...
} Foo;
...
won’t be able to include a [`ctypedef`](language_basics.html#ctypedef "language_basics.html#ctypedef") for it – but then,
it’s not necessary. 

See also use of [External extension types](extension_types.html#external-extension-types "extension_types.html#external-extension-types").
...
For a deeper introduction to pointers, you can read [this tutorial at tutorialspoint](https://www.tutorialspoint.com/cprogramming/c_pointers.htm "https://www.tutorialspoint.com/cprogramming/c_pointers.htm"). For differences between
Cython and C syntax for manipulating pointers, see [Statements and expressions](language_basics.html#statements-and-expressions "language_basics.html#statements-and-expressions").

### Accessing Python/C API routines[¶](#accessing-python-c-api-routines "Link to this heading")
...
[https://github.com/cython/cython/tree/master/Cython/Includes/cpython](https://github.com/cython/cython/tree/master/Cython/Includes/cpython "https://github.com/cython/cython/tree/master/Cython/Includes/cpython")
...
Cython predefines the name [`Py_ssize_t`](https://docs.python.org/3/c-api/intro.html#c.Py_ssize_t "(in Python v3.14)") for use with Python/C API routines. To
...
### Windows Calling Conventions[¶](#windows-calling-conventions "Link to this heading")
...
### Resolving naming conflicts - C name specifications[¶](#resolving-naming-conflicts-c-name-specifications "Link to this heading")
...
using the facilities described in [sharing declarations between Cython
modules](sharing_declarations.html#sharing-declarations "sharing_declarations.html#sharing-declarations"). Writing them into a `.pxd` file allows
...
C function called `yield()`. If you declare it as:
...
### Including verbatim C code[¶](#including-verbatim-c-code "Link to this heading")
...
## Using Cython Declarations from C[¶](#using-cython-declarations-from-c "Link to this heading")
...
[`cimport`](sharing_declarations.html#cimport "sharing_declarations.html#cimport") statement for that. Sharing Declarations Between Cython Modules.

### Public Declarations[¶](#public-declarations "Link to this heading")
...
`modulename.h` file is generated containing equivalent C declarations for
...
In this case, make sure to call [`Py_Initialize()`](https://docs.python.org/3/c-api/init.html#c.Py_Initialize "(in Python v3.14)") and [`Py_Finalize()`](https://docs.python.org/3/c-api/init.html#c.Py_Finalize "(in Python v3.14)").
For example, in the following snippet that includes `grail.h`:
...
use the [inittab mechanism](https://docs.python.org/3/c-api/import.html#c._inittab "https://docs.python.org/3/c-api/import.html#c._inittab")
...
#### C++ public declarations[¶](#c-public-declarations "Link to this heading")
...
### C API Declarations[¶](#c-api-declarations "Link to this heading")

The other way of making declarations available to C code is to declare them
with the [`api`](#api "#api") keyword. You can use this keyword with C functions and
extension types. A header file called `modulename_api.h` is produced
...
header and call the `import_modulename()` function. The other functions
...
library or executable, then `import_modulename()` function needs to be
...
the `import_modulename()` function before the api call which crashes.
...
Using the [`api`](#api "#api") method does not require the C code using the
...
[`Py_Initialize()`](https://docs.python.org/3/c-api/init.html#c.Py_Initialize "(in Python v3.14)") and [`Py_Finalize()`](https://docs.python.org/3/c-api/init.html#c.Py_Finalize "(in Python v3.14)"); if you experience a segmentation fault in
the call to `import_modulename()`, it is likely that this wasn’t done.

You can use both [`public`](extension_types.html#public "extension_types.html#public") and [`api`](#api "#api") on the same function to
...
`foo.spam_api.h` and an importing function called
`import_foo__spam()`.

### Multiple public and API declarations[¶](#multiple-public-and-api-declarations "Link to this heading")

You can declare a whole group of items as [`public`](extension_types.html#public "extension_types.html#public") and/or
[`api`](#api "#api") all at once by enclosing them in a [`cdef`](language_basics.html#cdef "language_basics.html#cdef") block, for
example,:
...
[Sharing Declarations Between Cython Modules](sharing_declarations.html#sharing-declarations "sharing_declarations.html#sharing-declarations")) to make the module’s public interface
available by all three methods.

### Acquiring and Releasing the GIL[¶](#acquiring-and-releasing-the-gil "Link to this heading")

Cython provides facilities for acquiring and releasing the
Global Interpreter Lock (GIL) (see [our glossary](glossary.html#term-Global-Interpreter-Lock-or-GIL "glossary.html#term-Global-Interpreter-Lock-or-GIL") or [external documentation](https://docs.python.org/dev/glossary.html#term-global-interpreter-lock "https://docs.python.org/dev/glossary.html#term-global-interpreter-lock")).
...
#### Releasing the GIL[¶](#releasing-the-gil "Link to this heading")
...
#### Acquiring the GIL[¶](#acquiring-the-gil "Link to this heading")
...
[PyEval\_InitThreads()](https://docs.python.org/dev/c-api/init.html#c.PyEval_InitThreads "https://docs.python.org/dev/c-api/init.html#c.PyEval_InitThreads").
If you’re already using [cython.parallel](parallelism.html#parallel "parallelism.html#parallel") in your module, this will already have been taken care of.
...
#### Conditional Acquiring / Releasing the GIL[¶](#conditional-acquiring-releasing-the-gil "Link to this heading")
...
for fused types (see [Conditional GIL Acquiring / Releasing](fusedtypes.html#fused-gil-conditional "fusedtypes.html#fused-gil-conditional")).:
...
### Declaring a function as callable without the GIL[¶](#declaring-a-function-as-callable-without-the-gil "Link to this heading")

You can specify [`nogil`](#nogil "#nogil") in a C function header or function type to
...
The `nogil` function annotation declares that it is safe
...
implicitly makes its signature [`nogil`](#nogil "#nogil").
...
* [Interfacing with External C Code](# "#")
  + [External declarations](#external-declarations "#external-declarations")
    - [Referencing C header files](#referencing-c-header-files "#referencing-c-header-files")
    - [Implementing functions in C](#implementing-functions-in-c "#implementing-functions-in-c")
    - [Styles of struct, union and enum declaration](#styles-of-struct-union-and-enum-declaration "#styles-of-struct-union-and-enum-declaration")
    - [Pointers](#pointers "#pointers")
    - [Accessing Python/C API routines](#accessing-python-c-api-routines "#accessing-python-c-api-routines")
    - [Special Types](#special-types "#special-types")
    - [Windows Calling Conventions](#windows-calling-conventions "#windows-calling-conventions")
    - [Resolving naming conflicts - C name specifications](#resolving-naming-conflicts-c-name-specifications "#resolving-naming-conflicts-c-name-specifications")
    - [Including verbatim C code](#including-verbatim-c-code "#including-verbatim-c-code")
  + [Using Cython Declarations from C](#using-cython-declarations-from-c "#using-cython-declarations-from-c")
    - [Public Declarations](#public-declarations "#public-declarations")
      * [C++ public declarations](#c-public-declarations "#c-public-declarations")
    - [C API Declarations](#c-api-declarations "#c-api-declarations")
    - [Multiple public and API declarations](#multiple-public-and-api-declarations "#multiple-public-and-api-declarations")
    - [Acquiring and Releasing the GIL](#acquiring-and-releasing-the-gil "#acquiring-and-releasing-the-gil")
      * [Releasing the GIL](#releasing-the-gil "#releasing-the-gil")
      * [Acquiring the GIL](#acquiring-the-gil "#acquiring-the-gil")
      * [Conditional Acquiring / Releasing the GIL](#conditional-acquiring-releasing-the-gil "#conditional-acquiring-releasing-the-gil")
    - [Declaring a function as callable without the GIL](#declaring-a-function-as-callable-without-the-gil "#declaring-a-function-as-callable-without-the-gil")

#### Previous topic

[Sharing Declarations Between Cython Modules](sharing_declarations.html "previous chapter")

#### Next topic

[Source Files and Compilation](source_files_and_compilation.html "next chapter")

### This Page

* [Show Source](../../_sources/src/userguide/external_C_code.rst.txt "../../_sources/src/userguide/external_C_code.rst.txt")
