...
Cython for NumPy users — Cython 3.2.0b1 documentation
...
[here](/en/stable/src/userguide/numpy_tutorial.html "/en/stable/src/userguide/numpy_tutorial.html").

### Navigation

* [next](numpy_ufuncs.html "Creating Numpy ufuncs")
* [previous](debugging.html "Debugging your Cython program") |
...
# Cython for NumPy users[¶](#cython-for-numpy-users "Link to this heading")
...
* Kurt Smith’s [video tutorial of Cython at SciPy 2015](https://www.youtube.com/watch?v=gMvkiQ-gOW8&t=4730s&ab_channel=Enthought "https://www.youtube.com/watch?v=gMvkiQ-gOW8&t=4730s&ab_channel=Enthought").
  The slides and notebooks of this talk are [on github](https://github.com/kwmsmith/scipy-2015-cython-tutorial "https://github.com/kwmsmith/scipy-2015-cython-tutorial").
* Basic Cython documentation (see [Cython front page](https://cython.readthedocs.io/en/latest/index.html "https://cython.readthedocs.io/en/latest/index.html")).

## Cython at a glance[¶](#cython-at-a-glance "Link to this heading")
...
[limitations](limitations.html#cython-limitations "limitations.html#cython-limitations").

## Your Cython environment[¶](#your-cython-environment "Link to this heading")
...
1. The [SAGE](https://sagemath.org "https://sagemath.org") mathematics software system provides
   excellent support for using Cython and NumPy from an interactive command
   line or through a notebook interface (like
   Maple/Mathematica). See [this documentation](https://doc.sagemath.org/html/en/developer/coding_in_cython.html "https://doc.sagemath.org/html/en/developer/coding_in_cython.html").
2. Cython can be used as an extension within a Jupyter notebook,
   ...
   [Using the Jupyter Notebook](../quickstart/build.html#jupyter-notebook "../quickstart/build.html#jupyter-notebook").
3. A version of pyximport is shipped with Cython,
   so that you can import pyx-files dynamically into Python and
   have them compiled automatically (See [Compiling with pyximport](source_files_and_compilation.html#pyximport "source_files_and_compilation.html#pyximport")).
4. Cython supports setuptools so that you can very easily create build scripts
   ...
   See [Basic setup.py](source_files_and_compilation.html#basic-setup-py "source_files_and_compilation.html#basic-setup-py").
...
otherwise, see [the installation page](../quickstart/install.html#install "../quickstart/install.html#install").
...
### Manual compilation[¶](#manual-compilation "Link to this heading")
...
### Compilation using setuptools[¶](#compilation-using-setuptools "Link to this heading")
...
## The first Cython program[¶](#the-first-cython-program "Link to this heading")

You can easily execute the code of this tutorial by
downloading [the Jupyter notebook](https://github.com/cython/cython/blob/master/docs/examples/userguide/numpy_tutorial/numpy_and_cython.ipynb "https://github.com/cython/cython/blob/master/docs/examples/userguide/numpy_tutorial/numpy_and_cython.ipynb").
...
This should be compiled to produce `compute_cy.so` for Linux systems
...
compute\_typed.py[¶](#id1 "Link to this code")
...
![../../_images/compute_typed_py_html.png](../../_images/compute_typed_py_html.png)

compute\_typed.pyx[¶](#id2 "Link to this code")
...
![../../_images/compute_typed_pyx_html.png](../../_images/compute_typed_pyx_html.png)
...
## Efficient indexing with memoryviews[¶](#efficient-indexing-with-memoryviews "Link to this heading")
...
We do this with a memoryview. There is [a page in the Cython documentation](memoryviews.html#memoryviews "memoryviews.html#memoryviews") dedicated to it.
...
compute\_memview.py[¶](#id3 "Link to this code")
...
compute\_memview.pyx[¶](#id4 "Link to this code")
...
with Python arrays. Check out the [memoryview page](memoryviews.html#memoryviews "memoryviews.html#memoryviews") to
...
mode in many ways, see [Compiler directives](source_files_and_compilation.html#compiler-directives "source_files_and_compilation.html#compiler-directives") for more
information.
...
## Declaring the NumPy arrays as contiguous[¶](#declaring-the-numpy-arrays-as-contiguous "Link to this heading")
...
all about, you can see [this answer on StackOverflow](https://stackoverflow.com/questions/26998223/what-is-the-difference-between-contiguous-and-non-contiguous-arrays "https://stackoverflow.com/questions/26998223/what-is-the-difference-between-contiguous-and-non-contiguous-arrays").
...
## Making the function cleaner[¶](#making-the-function-cleaner "Link to this heading")
...
You can learn more about it at [this section of the documentation](fusedtypes.html#fusedtypes "fusedtypes.html#fusedtypes").
...
## Using multiple threads[¶](#using-multiple-threads "Link to this heading")
...
parallelism in [Using Parallelism](parallelism.html#parallel "parallelism.html#parallel"). Since we do elementwise operations, we can easily
...
The GIL must be released (see [Releasing the GIL](external_C_code.html#nogil "external_C_code.html#nogil")), so this is why we
declare our `clip()` function `nogil`.
...
The need to check here is a bug with functions returning a fused type (see [#5586](https://github.com/cython/cython/issues/5586 "https://github.com/cython/cython/issues/5586") for the details).
To avoid acquiring the GIL, the function is declared as `noexcept` or `@cython.exceptval(check=False)`. See the [Error return values](language_basics.html#error-return-values "language_basics.html#error-return-values") section for more details.
...
* If you want to learn how to make use of [BLAS](https://www.netlib.org/blas/ "https://www.netlib.org/blas/")
  or [LAPACK](https://www.netlib.org/lapack/ "https://www.netlib.org/lapack/") with Cython, you can watch
  [the presentation of Ian Henriksen at SciPy 2015](https://www.youtube.com/watch?v=R4yB-8tB0J0&t=693s&ab_channel=Enthought "https://www.youtube.com/watch?v=R4yB-8tB0J0&t=693s&ab_channel=Enthought").
* If you want to learn how to use Pythran as backend in Cython, you
  can see how in [Pythran as a NumPy backend](numpy_pythran.html#numpy-pythran "numpy_pythran.html#numpy-pythran").
  Note that using Pythran only works with the
  [old buffer syntax](../tutorial/numpy.html#working-numpy "../tutorial/numpy.html#working-numpy") and not yet with memoryviews.
...
* [Cython for NumPy users](# "#")
  + [Cython at a glance](#cython-at-a-glance "#cython-at-a-glance")
  + [Your Cython environment](#your-cython-environment "#your-cython-environment")
  + [Installation](#installation "#installation")
  + [Compilation](#compilation "#compilation")
    - [Manual compilation](#manual-compilation "#manual-compilation")
    - [Compilation using setuptools](#compilation-using-setuptools "#compilation-using-setuptools")
  + [The first Cython program](#the-first-cython-program "#the-first-cython-program")
  + [Adding types](#adding-types "#adding-types")
  + [Efficient indexing with memoryviews](#efficient-indexing-with-memoryviews "#efficient-indexing-with-memoryviews")
  + [Tuning indexing further](#tuning-indexing-further "#tuning-indexing-further")
  + [Declaring the NumPy arrays as contiguous](#declaring-the-numpy-arrays-as-contiguous "#declaring-the-numpy-arrays-as-contiguous")
  + [Making the function cleaner](#making-the-function-cleaner "#making-the-function-cleaner")
  + [More generic code](#more-generic-code "#more-generic-code")
  + [Using multiple threads](#using-multiple-threads "#using-multiple-threads")
  + [Where to go from here?](#where-to-go-from-here "#where-to-go-from-here")

#### Previous topic

[Debugging your Cython program](debugging.html "previous chapter")

#### Next topic

[Creating Numpy ufuncs](numpy_ufuncs.html "next chapter")

### This Page

* [Show Source](../../_sources/src/userguide/numpy_tutorial.rst.txt "../../_sources/src/userguide/numpy_tutorial.rst.txt")
