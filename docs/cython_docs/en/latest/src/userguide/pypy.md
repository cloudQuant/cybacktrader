...
Porting Cython code to PyPy — Cython 3.2.0b1 documentation
...
[here](/en/stable/src/userguide/pypy.html "/en/stable/src/userguide/pypy.html").

### Navigation

* [next](limited_api.html "The Limited API and Stable ABI")
* [previous](fusedtypes.html "Fused Types (Templates)") |
...
# Porting Cython code to PyPy[¶](#porting-cython-code-to-pypy "Link to this heading")

Cython has basic support for cpyext, the layer in
[PyPy](https://pypy.org/ "https://pypy.org/") that emulates CPython’s C-API. This is
...
## Reference counts[¶](#reference-counts "Link to this heading")
...
## Object lifetime[¶](#object-lifetime "Link to this heading")
...
## Borrowed references and data pointers[¶](#borrowed-references-and-data-pointers "Link to this heading")
...
char\* buffer of a byte string. In PyPy, this will only work as
...
borrowed references, e.g. [`PyTuple_GET_ITEM()`](https://docs.python.org/3/c-api/tuple.html#c.PyTuple_GET_ITEM "(in Python v3.14)") and similar functions,
...
to [`Py_INCREF()`](https://docs.python.org/3/c-api/refcounting.html#c.Py_INCREF "(in Python v3.14)") when getting the reference and [`Py_DECREF()`](https://docs.python.org/3/c-api/refcounting.html#c.Py_DECREF "(in Python v3.14)")
when done with it to convert it into an owned reference.

## Builtin types, slots and fields[¶](#builtin-types-slots-and-fields "Link to this heading")

The following builtin types are not currently available in cpyext in
form of their C level representation: [`PyComplexObject`](https://docs.python.org/3/c-api/complex.html#c.PyComplexObject "(in Python v3.14)"),
[`PyFloatObject`](https://docs.python.org/3/c-api/float.html#c.PyFloatObject "(in Python v3.14)") and `PyBoolObject`.
...
of [`PyLongObject`](https://docs.python.org/3/c-api/long.html#c.PyLongObject "(in Python v3.14)") or the `allocated` field of the
[`PyListObject`](https://docs.python.org/3/c-api/list.html#c.PyListObject "(in Python v3.14)") struct etc. Although the `ob_size` field of
containers (used by the [`Py_SIZE()`](https://docs.python.org/3/c-api/structures.html#c.Py_SIZE "(in Python v3.14)") macro) is available, it is
...
## GIL handling[¶](#gil-handling "Link to this heading")

Currently, the GIL handling function [`PyGILState_Ensure()`](https://docs.python.org/3/c-api/init.html#c.PyGILState_Ensure "(in Python v3.14)") is not
...
See [PyGILState\_Ensure should not deadlock if GIL already held](https://foss.heptapod.net/pypy/pypy/-/issues/1778 "https://foss.heptapod.net/pypy/pypy/-/issues/1778").

## Efficiency[¶](#efficiency "Link to this heading")
...
pointer in CPython. A visible example is [`PyTuple_GET_ITEM()`](https://docs.python.org/3/c-api/tuple.html#c.PyTuple_GET_ITEM "(in Python v3.14)").

Some more high-level functions may also show entirely different performance
characteristics, e.g. [`PyDict_Next()`](https://docs.python.org/3/c-api/dict.html#c.PyDict_Next "(in Python v3.14)") for dict iteration. While
...
## Known problems[¶](#known-problems "Link to this heading")
...
## Bugs and crashes[¶](#bugs-and-crashes "Link to this heading")
...
* [Porting Cython code to PyPy](# "#")
  ...
  + [Object lifetime](#object-lifetime "#object-lifetime")
  + [Borrowed references and data pointers](#borrowed-references-and-data-pointers "#borrowed-references-and-data-pointers")
  + [Builtin types, slots and fields](#builtin-types-slots-and-fields "#builtin-types-slots-and-fields")
  + [GIL handling](#gil-handling "#gil-handling")
...
[Fused Types (Templates)](fusedtypes.html "previous chapter")

#### Next topic

[The Limited API and Stable ABI](limited_api.html "next chapter")

### This Page

* [Show Source](../../_sources/src/userguide/pypy.rst.txt "../../_sources/src/userguide/pypy.rst.txt")
