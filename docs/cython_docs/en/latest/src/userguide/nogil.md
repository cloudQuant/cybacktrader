...
Cython and the GIL — Cython 3.2.0b1 documentation
...
[here](/en/stable/src/userguide/nogil.html "/en/stable/src/userguide/nogil.html").

### Navigation

* [next](freethreading.html "Free threading")
* [previous](numpy_pythran.html "Pythran as a Numpy backend") |
...
# Cython and the GIL[¶](#cython-and-the-gil "Link to this heading")

Python has a global lock ([the GIL](glossary.html#term-Global-Interpreter-Lock-or-GIL "glossary.html#term-Global-Interpreter-Lock-or-GIL"))
...
1. Using [Cython’s parallelism mechanism](parallelism.html#parallel "parallelism.html#parallel"). The contents of
...
2. If you want other (external) Python threads to be able to run at the same time.
   ...
   1. if you have a large computationally/IO-intensive block that doesn’t need the
   ...
   2. (very, very occasionally) in long-running Cython code that never calls into the
...
## Marking functions as able to run without the GIL[¶](#marking-functions-as-able-to-run-without-the-gil "Link to this heading")

You can mark a whole function (either a Cython function or an [external function](external_C_code.html#nogil "external_C_code.html#nogil")) as
...
## Releasing (and reacquiring) the GIL[¶](#releasing-and-reacquiring-the-gil "Link to this heading")
...
### Conditionally acquiring the GIL[¶](#conditionally-acquiring-the-gil "Link to this heading")

It’s possible to release the GIL based on a compile-time condition.
This is most often used when working with [Fused Types (Templates)](fusedtypes.html#fusedtypes "fusedtypes.html#fusedtypes")
...
## Exceptions and the GIL[¶](#exceptions-and-the-gil "Link to this heading")
...
## Don’t use the GIL as a lock[¶](#don-t-use-the-gil-as-a-lock "Link to this heading")
...
1. that future improvements in the Python interpreter may destroy
   your “locking”.
2. Second, that the GIL can be released if any Python code is
...
* [Cython and the GIL](# "#")
  + [Marking functions as able to run without the GIL](#marking-functions-as-able-to-run-without-the-gil "#marking-functions-as-able-to-run-without-the-gil")
  + [Releasing (and reacquiring) the GIL](#releasing-and-reacquiring-the-gil "#releasing-and-reacquiring-the-gil")
    - [Conditionally acquiring the GIL](#conditionally-acquiring-the-gil "#conditionally-acquiring-the-gil")
  + [Exceptions and the GIL](#exceptions-and-the-gil "#exceptions-and-the-gil")
  + [Don’t use the GIL as a lock](#don-t-use-the-gil-as-a-lock "#don-t-use-the-gil-as-a-lock")

#### Previous topic

[Pythran as a Numpy backend](numpy_pythran.html "previous chapter")

#### Next topic

[Free threading](freethreading.html "next chapter")

### This Page

* [Show Source](../../_sources/src/userguide/nogil.rst.txt "../../_sources/src/userguide/nogil.rst.txt")
