...
Memory Allocation — Cython 3.1.4 documentation
...
`malloc()` or [`PyMem_Malloc()`](https://docs.python.org/3/c-api/memory.html#c.PyMem_Malloc "(in Python v3.13)") must be manually released
with a corresponding call to `free()` or [`PyMem_Free()`](https://docs.python.org/3/c-api/memory.html#c.PyMem_Free "(in Python v3.13)")
